...
class GeneticFLServer:
    def __init__(
        ...
        gl_val_loader: Any,
        test_torch: Any,
        initial_hyperparams: List[List[float]]
    ):
        self.cfg = cfg
        self.model = model
        self.model_name = model_name
        ...
        self.gl_val_loader = gl_val_loader
        self.test_torch = test_torch
        self.hyperparams = initial_hyperparams  # 예시: [[0.001, 128], [0.005, 64], [0.01, 32]]
        self.client_updates: List[Dict] = []  # 클라이언트 업데이트 저장 리스트
        self.num_rounds = int(cfg.get("num_rounds", 20))
        self.task_id = os.environ.get("TASK_ID", "default_task")
        self.server = server_utils.FLServerStatus()  # 글로벌 모델 버전 등 상태 관리 객체
        self.init_model = model  # FLServer의 흐름과 맞추기 위한 초기 모델 저장

    def get_model_weights(self) -> Dict:
        """모델의 현재 가중치(state_dict)를 반환합니다."""
        return self.model.state_dict()
    
    def set_model_weights(self, new_state_dict: Dict) -> None:
        """새로운 가중치를 모델에 설정합니다."""
        self.model.load_state_dict(new_state_dict)
    
    def send_to_clients(self, broadcast_data: Dict) -> None:
        """모델 및 하이퍼파라미터 정보를 클라이언트에게 전송합니다."""
        print("Broadcasting model and hyperparameters to clients:")
        print(broadcast_data)
    ...
    def collect_client_updates(self) -> List[Dict]:
        """
        클라이언트 업데이트를 수집합니다.
        실제 환경에서는 네트워크 통신이나 메시지 큐를 활용하며,
        여기서는 더미 업데이트를 반환합니다.
        """
        dummy_update = {
            "weights": self.get_model_weights(),
            "loss": 0.25,  # 실제 손실 값 사용
            "hyperparam": self.hyperparams[0]  # 예: [0.001, 128]
        }
        print("Collecting client updates (dummy update)...")
        return [dummy_update]
    ...
    def aggregate_client_updates(self, client_updates: List[Dict]) -> Dict:
        """
        클라이언트 업데이트를 집계하고, 하이퍼파라미터를 클러스터링한 후 진화시킵니다.
        Returns:
            Dict: 업데이트된 글로벌 모델 가중치
        """
        total_weights = None
        losses = []
        hyperparams_list = []  # 각 항목은 [lr, bs] 형태
        ...
        for update in client_updates:
            ...
            hyperparams_list.append(update["hyperparam"])
            if total_weights is None:
                total_weights = {k: v.clone() for k, v in weights.items()}
            else:
                for k in total_weights:
        ...
        # 하이퍼파라미터 클러스터링 (DBSCAN)
        hyperparams_array = np.array(hyperparams_list)
        scaled_array = hyperparams_array.copy()
        # 배치크기는 로그 변환하여 스케일 보정 (0 이상의 값이어야 함)
        scaled_array[:, 1] = np.log(scaled_array[:, 1])
        dbscan = DBSCAN(eps=0.1, min_samples=2)
        clusters = dbscan.fit_predict(scaled_array)
        logging.info("DBSCAN clusters: %s", clusters)
        ...
        # 클러스터링으로 새로운 후보가 없으면 원래 하이퍼파라미터 유지
        self.hyperparams = new_hyperparams if new_hyperparams else self.hyperparams
        logging.info("Evolved hyperparameters after clustering: %s", self.hyperparams)
    ...
    def evaluate_global_model(self) -> Tuple[float, float, Any]:
        """현재 글로벌 모델을 검증 셋으로 평가합니다."""
        loss, acc, metrics = self.test_torch(self.model, self.gl_val_loader, self.cfg)
        return loss, acc, metrics
    
    def broadcast_model_and_hyperparams(self) -> None:
        """글로벌 모델 가중치와 하이퍼파라미터를 클라이언트에 방송합니다."""
        broadcast_data = {
            "model_weights": self.get_model_weights(),
            "hyperparams": self.hyperparams
        }
    ...
    def train_round(self, round_number: int) -> None:
        """하나의 훈련 라운드를 수행합니다."""
        logging.info("=== Starting Training Round %d ===", round_number)
        self.broadcast_model_and_hyperparams()
        client_updates = self.collect_client_updates()
        new_global_weights = self.aggregate_client_updates(client_updates)
        self.set_model_weights(new_global_weights)
        loss, acc, _ = self.evaluate_global_model()
        logging.info("Round %d - Global Model => Loss: %.4f, Accuracy: %.4f", round_number, loss, acc)
        eval_payload = {
            "round": round_number,
            "loss": loss,
            "accuracy": acc,
    ...
    def init_gl_model_registration(self, model: Any, gl_model_name: str) -> str:
        """
        글로벌 모델 등록 또는 로드를 위한 초기 작업을 진행합니다.
        Returns:
            str: 등록된 모델 이름
        """
        logging.info("last_gl_model_v: %s", self.server.last_gl_model_v)
        if not model:
            logging.info("Creating initial global model")
            init_model, model_name = self.init_model, self.model_name
            print(f"init_gl_model_name: {model_name}")
            return model_name
        else:
            logging.info("Loading last global model")
            print(f"last_gl_model_name: {gl_model_name}")
            return gl_model_name
    
    def register_and_upload_model(self) -> None:
        """
        최종 글로벌 모델을 등록하고, S3에 업로드합니다.
        """
        self.server.gl_model_v = self.server.last_gl_model_v + 1
        fl_start_time = time.time()
        gl_model_name = self.init_gl_model_registration(self.next_model, self.next_model_name)
        ...
        json_all_time_result = json.dumps(server_all_time_result)
        logging.info("server_operation_time - %s", json_all_time_result)
        server_api.ServerAPI(self.task_id).put_server_time_result(json_all_time_result)
        ...
        file_path = os.path.join(".", global_model_file_name)
        if not os.path.exists(file_path):
            logging.error("Global model file not found: %s", file_path)
            raise FileNotFoundError(f"Global model file not found: {file_path}")
        
        try:
            server_utils.upload_model_to_bucket(self.task_id, global_model_file_name)
            logging.info("Uploaded %s model to S3 bucket", global_model_file_name)
        except Exception as upload_err:
            logging.error("Error during S3 upload:", exc_info=upload_err)
            raise upload_err
    ...
    def start(self) -> None:
        """
        FL 서버를 시작하여 모델 다운로드, 글로벌 모델 등록, 훈련 라운드 실행, 모델 업로드까지 전체 플로우를 수행합니다.
        """
        today_time = datetime.datetime.today().strftime('%Y-%m-%d %H-%M-%S')
        try:
            # 글로벌 모델 다운로드 (S3 또는 로컬)
            self.next_model, self.next_model_name, self.server.last_gl_model_v = server_utils.model_download_s3(
                self.task_id, self.model_type, self.init_model
            )
            self.server.gl_model_v = self.server.last_gl_model_v + 1
            inform_payload = {
                "S3_bucket": "fl-gl-model",
                "Last_GL_Model": f"gl_model_{self.server.last_gl_model_v}_V",
                "FLServer_start": today_time,
                "FLSeReady": True,
                "GL_Model_V": self.server.gl_model_v
            }
            ...
            logging.error("Error during model registration and upload: %s", e, exc_info=True)
            data_inform = {"FLSeReady": False}
            server_api.ServerAPI(self.task_id).put_server_status(json.dumps(data_inform))
        finally:
            logging.info("Closing server after registration")
            server_api.ServerAPI(self.task_id).put_fl_round_fin()
            logging.info("Global model version upgrade complete")
